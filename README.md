...
routed-ingress topology, the step-by-step walk-through of what the
chained CNI plugin does during pod setup, the inbound and egress
traffic paths, and a short note on the `/32` transparent-source
fallback that still lives in `src/proxy/local_source.{hpp,cpp}` but
is off by default on the current deployment.
...
```bash
ssh $K3S_HOST kubectl delete -k /tmp/inline-proxy-build/deploy/base
```
...
| `INLINE_PROXY_USE_PROXY_SOURCE` | `1` in manifest | Upstream socket binds to the proxy's own IP instead of the original client. Required on k3s because flannel rejects spoofed pod-to-pod sources. Unset to restore original-client-source binds (requires `SKIP_LOCAL_SOURCE=0`). |
| `INLINE_PROXY_SKIP_LOCAL_SOURCE` | `1` in manifest | Skips the `/32` client-IP localisation on `wan_*` (the fallback path in `src/proxy/local_source.{hpp,cpp}`). Pairs with `USE_PROXY_SOURCE=1`. Leave set under the routed topology. |

For the full topology walk-through, the inbound/egress traffic paths, and the note on the `/32` fallback, see [`docs/architecture.md`](docs/architecture.md).
